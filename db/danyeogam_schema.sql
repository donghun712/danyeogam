...
CREATE TABLE IF NOT EXISTS title_definition (
    ...
    condition_type          VARCHAR(40)     NOT NULL DEFAULT 'VISIT_COUNT',
    target_region_level     VARCHAR(20)     NULL,
    classification_level    TINYINT UNSIGNED NULL,
    classification_codes    VARCHAR(255)    NULL,
    required_visit_count    INT UNSIGNED    NULL,
    required_progress_percent DECIMAL(5, 2) NULL,
    ...
    CONSTRAINT ck_title_definition_condition_type
        CHECK (condition_type IN (
            'VISIT_COUNT', 'DISTINCT_REGION_COUNT',
            'CLASSIFICATION_VISIT_COUNT', 'REGION_PROGRESS_PERCENT'
        )),
    CONSTRAINT ck_title_definition_target_region_level
        CHECK (target_region_level IS NULL OR target_region_level IN ('PROVINCE', 'CITY_COUNTY')),
    CONSTRAINT ck_title_definition_classification_level
        CHECK (classification_level IS NULL OR classification_level IN (2, 3)),
    CONSTRAINT ck_title_definition_condition_values
        CHECK (
            (condition_type = 'VISIT_COUNT' AND scope_type = 'GLOBAL'
                AND required_visit_count > 0 AND target_region_level IS NULL
                AND classification_level IS NULL AND classification_codes IS NULL
                AND required_progress_percent IS NULL)
            OR
            (condition_type = 'DISTINCT_REGION_COUNT' AND scope_type = 'GLOBAL'
                AND required_visit_count > 0 AND target_region_level IS NOT NULL
                AND classification_level IS NULL AND classification_codes IS NULL
                AND required_progress_percent IS NULL)
            OR
            (condition_type = 'CLASSIFICATION_VISIT_COUNT' AND scope_type = 'GLOBAL'
                AND required_visit_count > 0 AND target_region_level IS NULL
                AND classification_level IS NOT NULL AND classification_codes IS NOT NULL
                AND required_progress_percent IS NULL)
            OR
            (condition_type = 'REGION_PROGRESS_PERCENT' AND scope_type = 'REGION'
                AND required_visit_count IS NULL AND target_region_level IS NULL
                AND classification_level IS NULL AND classification_codes IS NULL
                AND required_progress_percent > 0 AND required_progress_percent <= 100)
        ),
...
CREATE TABLE IF NOT EXISTS actor_title (
    ...
    verification_attempt_id BIGINT UNSIGNED NULL,
    ...
    CONSTRAINT fk_actor_title_definition
        ...
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT fk_actor_title_verification_attempt
        FOREIGN KEY (verification_attempt_id) REFERENCES verification_attempt (id)
        ON UPDATE RESTRICT ON DELETE SET NULL,
    INDEX ix_actor_title_awarded (actor_id, awarded_at),
    INDEX ix_actor_title_verification_attempt (actor_id, verification_attempt_id)
) ENGINE = InnoDB;
...
VALUES ('2.1.0', 'Danyeogam schema with TourAPI classifications, favorites, and titles')
