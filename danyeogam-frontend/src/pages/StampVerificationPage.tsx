...
export function StampVerificationPage() {
  ...
  const { phase, result, errorCode, isBusy, start } = useStampVerification(
    spotId ?? -1,
  );
    ...
    <FullPageLayout title="스탬프 인증">
      <div className={styles.container}>
        ...
        {phase === "result" && result?.status === "VERIFIED_NEW" && (
          <div className={styles.center}>
            <p className="text-display">방문 완료!</p>
            <p className="text-body">새로운 스탬프를 획득했습니다.</p>
          ...
          <div className={styles.center}>
            <p className="text-body">
              {result.distanceMeters !== null
                ? `관광지까지 약 ${Math.round(result.distanceMeters)}m 남았어요.`
